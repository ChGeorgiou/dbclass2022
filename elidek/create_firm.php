...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_equity)){
        $equity_err = "Please enter the equity.";
    } else{
        $equity = $input_equity;
    }
    ...
    if(empty($name_err) && empty($city_err) && empty($street_err) && empty($number_err) && empty($code_err) && empty($abr_err) && empty($equity_err)){
        ...
        $sql = "INSERT INTO organisation (organisation_name, city, street_name, street_number, postal_code, abbreviation, organisation_type, budget1) VALUES (?, ?, ?, ?, ?, ?, 'Firm', ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssiiss", $param_name, $param_city, $param_street, $param_number, $param_code, $param_abr, $param_equity);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <a href="index_firm.php" class="btn btn-secondary ml-2">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
