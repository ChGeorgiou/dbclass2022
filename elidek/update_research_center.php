...
if(isset($_POST["organisation_id"]) && !empty($_POST["organisation_id"])){
   ...
   // Validate organisation_name
    ...
    if(empty($input_equity)){
        $equity_err = "Please enter ministry budget.";
    } else{
        $equity = $input_equity;
    }
    ...
    if(empty($input_budget)){
        $budget_err = "Please enter private actions budget.";
    } else{
        $budget = $input_budget;
    }
    ...
    if(empty($name_err) && empty($city_err) && empty($street_err) && empty($number_err) && empty($code_err) && empty($abr_err) && empty($equity_err)){
        ...
        $sql = "UPDATE organisation SET organisation_name=?, city=?, street_name=?, street_number=?, postal_code=?, abbreviation=?, budget1=?, budget2=? WHERE organisation_id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssiissii", $param_name, $param_city, $param_street, $param_number, $param_code, $param_abr, $param_equity, $param_budget, $param_id);
    ...
    if(isset($_GET["organisation_id"]) && !empty(trim($_GET["organisation_id"]))){
        ...
        $sql = "SELECT organisation_name, city, street_name, street_number, postal_code, abbreviation, organisation_type, budget1, budget2 FROM organisation WHERE organisation_id = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
